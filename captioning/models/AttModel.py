...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.seq_length = getattr(opt, 'max_length', 32) or opt.seq_length # maximum sample length
    ...
    def _forward(self, images, seq):
        feats = self.resnet(images)
        fc_feats = feats.mean(3).mean(2)
        att_feats = self.adaptive_pool(feats).reshape(feats.shape[0], -1, feats.shape[1])
        att_masks = None
    ...
    def _sample_beam(self, img_feats, opt={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = utils.repeat_tensors(beam_size, [p_fc_feats, p_att_feats,
                                                                                              pp_att_feats,
                                                                                              p_att_masks])
    ...
    def _sample(self, img_feats, opt={}):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(img_feats, opt)
        self.resnet.eval()
        with torch.no_grad():
            feats = self.resnet(img_feats)
            fc_feats = feats.mean(3).mean(2)
