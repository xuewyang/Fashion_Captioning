    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if divm <= t <= self.seq_length + divm - 1:
                    # add diversity
                    ...
                    # infer new beams
                    beam_seq_table[divm], beam_seq_logprobs_table[divm], beam_logprobs_sum_table[divm], state_table[
                        divm] = beam_step(logprobs, unaug_logprobs, bdash, t - divm, beam_seq_table[divm],
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if divm <= t <= self.seq_length + divm - 1:
                    ...
                    # infer new beams
                    beam_seq_table[divm], beam_seq_logprobs_table[divm], beam_logprobs_sum_table[divm], state_table[
                        divm], candidates_divm = beam_step(logprobsf,
                                                           unaug_logprobsf,
                                                           bdash,
                                                           t - divm,
                                                           beam_seq_table[divm],
