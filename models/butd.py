...
class Attention(nn.Module):
    ...
    def __init__(self, features_dim, decoder_dim, attention_dim, dropout=0.5):
        ...
        self.features_att = weight_norm(nn.Linear(features_dim, attention_dim))  # FF to transform encoded image
        self.decoder_att = weight_norm(nn.Linear(decoder_dim, attention_dim))  # FF to transform decoder's output
        self.full_att = weight_norm(nn.Linear(attention_dim, 1))  # FF to calculate values to be softmax-ed
        self.relu = nn.ReLU()
...
class BUTD(nn.Module):
    """
    Bottom up top down attention model: https://arxiv.org/abs/1707.07998
    """
    ...
    def forward(self, images, encoded_captions, caption_lengths):
        ...
        image_features = out.permute(0, 2, 3, 1).reshape([batch_size, -1, self.features_dim])
        # (batch_size, encoded_image_size x encoded_image_size, 2048)
        ...
        # word embeddings to the top down attention model. Then pass the hidden state of the top down model and the
        # bottom up features to the attention block. The attention weighed bottom up features and hidden state of the
        # top down attention model are then passed to the language model
