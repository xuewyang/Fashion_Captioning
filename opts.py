...
def parse_opt():
    ...
    parser.add_argument('--model_path', type=str,
                        help='path for pretrained ResNet.', default='')
    parser.add_argument('--word_map_file', type=str,
                        help='path for word map.', default='')
    parser.add_argument('--model_folder', type=str,
                        help='base folder to save models.', default='')
    parser.add_argument('--checkpoint', type=str,
                        help='Resume training using checkpoint.', default='')
    parser.add_argument('--finetune_start_layer', type=int,
                        help='finetune resnet from this layer apart.', default=5)
    ...
    parser.add_argument('--epochs_since_improvement', type=int,
                        help='keeps track of number of epochs since theirs been an improvement in validation BLEU.',
                        default=0)
    parser.add_argument('--batch_size', type=int,
                        help='batch size for training.', default=145)
    parser.add_argument('--workers', type=int,
                        help='for data-loading; right now, only 1 works with h5py.', default=1)
    parser.add_argument('--print_freq', type=int,
                        help='print training/validation stats every __ batches.', default=50)
    parser.add_argument('--emb_dim', type=int,
                        help='dimension of word embeddings.', default=512)
    parser.add_argument('--attention_dim', type=int,
                        help='dimension of attention linear layers.', default=512)
    parser.add_argument('--decoder_dim', type=int,
                        help='dimension of decoder RNN.', default=512)
    parser.add_argument('--encoded_image_size', type=int,
                        help='encoded image size of encoder output.', default=14)

    parser.add_argument('--encoder_lr', type=float,
                        help='learning rate for encoder if fine-tuning.', default=1e-4)
    parser.add_argument('--lr', type=float,
                        help='learning rate for decoder.', default=1e-4)
    parser.add_argument('--alpha_c', type=float,
                        help='regularization parameter for doubly stochastic attention, as in the paper.', default=1)
    parser.add_argument('--dropout', type=float,
                        help='dropout rate.', default=0.5)
    parser.add_argument('--grad_clip', type=float,
                        help='clip gradients at an absolute value of.', default=5)
    parser.add_argument('--best_cider', type=float,
                        help='best cider saved.', default=0.)
    parser.add_argument('--fine_tune_encoder',
                        help='fine tune encoder or not.', action='store_true')
    ...
    parser.add_argument('--att_hid_size', type=int, default=512,
                        help='the hidden size of the attention MLP; only useful in show_attend_tell; '
    ...
    parser.add_argument('--train_num', type=int, default=883212,
                        help='number of training samples')
    parser.add_argument('--val_num', type=int, default=10000,
                        help='number of validation samples')
    parser.add_argument('--test_num', type=int, default=993193,
    ...
    parser.add_argument('--self_critical_after', type=int, default=-1,
                        help='After what epoch do we start finetuning the CNN? (-1 = disable; never finetune, '
                             '0 = finetune from start)')
    parser.add_argument('--seq_per_img', type=int, default=1,
                        help='number of captions to sample for each image during training. Done for efficiency since '
                             'CNN forward pass is expensive. E.g. coco has 5 sents/image, 1 for FACAD')
    ...
    parser.add_argument('--set_cfgs', dest='set_cfgs',
                        help='Set config keys. Key value sequence seperate by whitespace.'
                             'e.g. [key] [value] [key] [value]\n This has higher priority'
                             'than cfg file but lower than other args. (You can only overwrite'
                             'arguments that have alerady been defined in config file.)', default=[], nargs='+')
...
def add_eval_sample_opts(parser):
    ...
    parser.add_argument('--beam_size', type=int, default=1,
                        help='used when sample_method = greedy, indicates number of beams in beam search. '
                             'Usually 2 or 3 works well. More is not better. Set this to 1 for faster runtime but '
