...
def train(opt):
    ...
    try:
        while True:
            ...
            optimizer.zero_grad()
            model_out = dp_lw_model(img_feats, labels, masks, data['gts'], torch.arange(0, len(data['gts'])), sc_flag, struc_flag)
            ...
            if iteration % opt.losses_log_every == 0:
                if struc_flag:
                    print("iter {} (epoch {}), train_loss = {:.3f}, lm_loss = {:.3f}, struc_loss = {:.3f}, "
                          "time/batch = {:.3f}".format(iteration, epoch, train_loss, model_out['lm_loss'].mean().item(),
                                                       model_out['struc_loss'].mean().item(), end - start))
                elif not sc_flag:
                    print("iter {} (epoch {}), train_loss = {:.3f}, "
                          "time/batch = {:.3f}".format(iteration, epoch, train_loss, end - start))
                else:
                    print("iter {} (epoch {}), avg_reward = {:.3f}, "
                          "time/batch = {:.3f}".format(iteration, epoch, model_out['reward'].mean(), end - start))
            ...
            if (iteration % opt.save_checkpoint_every == 0 and not opt.save_every_epoch) or \
               (epoch_done and opt.save_every_epoch):
                ...
                if opt.save_history_ckpt:
                    utils.save_checkpoint(opt, model, infos, optimizer, append=str(epoch) if opt.save_every_epoch else
                    str(iteration))
